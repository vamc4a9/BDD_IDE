...
class StepDefinition 
	...
	public JLabel fn_AddTextLabel(JFrame frm, String sLabel, String sFont, int fSize, Dimension size, 
			Point iLocation) {
	...
	{ 
		if (e.getSource() == sub) { 
			...
			FindStepDef oFind = new FindStepDef();
			List<List<String>> rOutput = oFind.FindMatch(sXMLPath, tname.getText(), tParameters.getText());
	...
	public void DisplayStepDefs(List<List<String>> lData) 
	{ 
		JFrame f = new JFrame(); 
		Container c1 = f.getContentPane();

		// Frame Title 
		f.setTitle("Existing Step Definitions"); 

		// Column Names 
		String[] columnNames = { "Category", "StepDefinition"}; 

		String[][] array = new String[lData.size()][];
		for (int i = 0; i < lData.size(); i++) {
		    List<String> row = lData.get(i);
...
//		jTable.setBounds(30, 40, 200, 300); 
		...
		JLabel oTextLabel = new JLabel("Click on Yes to add '" + tname.getText() +"'"); 
		oTextLabel.setFont(new Font("Arial", Font.PLAIN, 14)); 
		oTextLabel.setSize(new Dimension(450, 40));
		System.out.println(jTable.getHeight() + 15);
		oTextLabel.setLocation(new Point(40, 70)); 
		c1.add(oTextLabel);
		

		// adding it to JScrollPane 
		JScrollPane sp = new JScrollPane(jTable); 
		sp.setLocation(new Point(10, 10));
		sp.setSize(new Dimension(300, 60));
		...
		jTable.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					...
					String sValue = jTable.getValueAt(row, column).toString();
					msgbox(sValue);
				}
			}
		});
