...
public class xml {
	...
	public String ReadNodeName(Document doc, String xPath) {
		NodeList node;
		try {
			XPathFactory xpathFactory = XPathFactory.newInstance();
			XPath xpath = xpathFactory.newXPath();
	...
	public List<String> fn_FindInXml(String xmlPath, String xPath) {
		NodeList node;
		try {
			Document doc = GetDocumentFromXml(xmlPath);
			XPathFactory xpathFactory = XPathFactory.newInstance();
			...
			for (int i = 0; i < node.getLength(); i++) {
				lOutput.add(ReadNodeName(doc, "//StepDefinition[@Statement='"+node.item(i).getTextContent()+"']/parent::*") + "::" + node.item(i).getTextContent());
			}
			return lOutput;
		} catch (Exception e) {
			return null;
